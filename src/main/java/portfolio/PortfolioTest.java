...
public class PortfolioTest {
	...
	private List<String> accountSummaryLines(ReceptiveAccount fromAccount) {
		AccountSummary summary = new AccountSummary(fromAccount);
	...
	private double accountTransferNet(ReceptiveAccount account) {
		TransferNet accountTransferNet = new TransferNet(account);
	...
	private double investmentNet(ReceptiveAccount account) {
		InvestmentNet accountInvestmentNet = new InvestmentNet(account);
	...
	private double investmentEarnings(ReceptiveAccount account) {
		InvestmentEarnings accountInvestmentEarnings = new InvestmentEarnings(account);
	...
	private List<String> portofolioTreeOf(Portfolio composedPortfolio,
		...
		PortfolioTreePrinter printer = new PortfolioTreePrinter (
				composedPortfolio,accountNames);
	...
	private List<String> reversePortofolioTreeOf(Portfolio composedPortfolio,
			Hashtable<SummarizingAccount, String> accountNames) {
		ReversePortfolioTreePrinter printer = new ReversePortfolioTreePrinter (
				composedPortfolio,accountNames);
	...
	@Test
	public void test27AccountSummaryWithInvestmentEarningsShouldNotTakeTooLong(){
		final ReceptiveAccount fromAccount = new ReceptiveAccount ();
		ReceptiveAccount toAccount = new ReceptiveAccount ();
		...
		new AssertShouldNotTakeMoreThan(1500) {
			@Override
			public void should() {
				lines.addAll(new AccountSummaryWithInvestmentEarnings(fromAccount).lines());
			}
		};
		...
		assertEquals("Extracción por 50.0", lines.get(1));
		assertEquals("Transferencia por -100.0", lines.get(2));
		assertEquals("Plazo fijo por 1000.0 durante 360 días a una tna de 0.1", lines.get(3));
		assertEquals("Ganancias por 100.0", lines.get(4));
	}

	@Test
	public void test28AccountSummaryWithInvestmentFullInfoShouldNotTakeTooLong(){
		final ReceptiveAccount fromAccount = new ReceptiveAccount ();
		ReceptiveAccount toAccount = new ReceptiveAccount ();
		final List<String> lines = new ArrayList<String>(); 
		...
		new AssertShouldNotTakeMoreThan(1500) {
			@Override
			public void should() {
				lines.addAll(new AccountSummaryWithAllInvestmentInformation(fromAccount).lines());
			}
		};
		...
		assertEquals("Plazo fijo por 1000.0 durante 360 días a una tna de 0.1", lines.get(3));
		assertEquals("Ganancias por 100.0", lines.get(4));
		assertEquals("Inversiones por 1000.0", lines.get(5));
	}
}
